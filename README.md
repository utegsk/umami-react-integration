...
Lightweight React component for [Umami Analytics](https://umami.is) — every `data-*` attribute Umami exposes, mapped to a typed prop, with first-class control over the recorder script that powers **heatmaps** and **replays**.
...
Then toggle heatmaps and/or replays on in your website settings in the Umami dashboard. Loading `recorder.js` is the client-side requirement; the dashboard decides what to do with the captured data.
